    ...
    ui(new Ui::board1)
{
 ...
 void board1::paintEvent(QPaintEvent *)//显示
 {
 ...
 void board1::mouseReleaseEvent(QMouseEvent *ev)
 {
   QPoint pt=ev->pos();
   int x=pt.x();
   int y=pt.y();
   ...
   if(flag2==-1)
   {
        ...
        if(ifmousepress&&_a[flag3]._drawed==0)
           ...
           if(ifmousepress&&_a[flag3]._drawed==1&&((_a[flag3]._color==1&&q%2==0)||(_a[flag3]._color==0&&q%2!=0)))
            {
               flag1=flag3;
                 flag2=flag3;
            }
 ...
 void board1::place_point(int a,int&x,int &y)
    {         switch (a=a+0) {
 ...
 int board1::mousepres2(int x, int y)
 {  if(y>l&&y<2*l)
    {
     if(x>l&&x<2*l)
      return 0;
 ...
 bool board1::rightprocess(bool a,int b,int x,int y)//y为flag2,x为flag3
 {
     if(a)
     {  if(_a[y]._type=="炮")
          {if(b==2)
 ...
 bool board1::getit( int a,int x,int y)
 ...
 int board1::ifpao(bool a, int x, int y)
 {  if(_a[x]._type!="炮")
      return 0;
    else{
     int t=0;
 ...
 int board1::pao_useed(int x, int y)//2表示炮符合吃子条件；3表示炮与点中的位置之间无其他棋子，4表示炮与点中位置相隔两个及以上的棋子,5表示两颗棋子不在同一行列
 {  int a,b,a1,b1;
     a=x%8;//x所在的列
       b=x/8;//x所在的行
        a1=y%8;//y所在的列
 ...
 void board1::deadchess(int a, int &x, int &y)
 {
              switch (a=a+0) {
 ...
 bool board1::rule()
 {
   ...
   if(t1==16)
   {
       p1.setText(QString("红方胜"));
       return true;
   }
   if(t2==16)
   {
      p1.setText(QString("黑方胜"));
       return true;
   }
...
void board1::on_pushButton_released()
{
w4.show();
